...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!user.password) return NextResponse.json({ message: "User password missing" }, { status: 400 });
    ...
    await Otp.findOneAndUpdate(
      { email },
      { code: otpCode, expiresAt: new Date(Date.now() + 10 * 60 * 1000), tempToken, attempts: 0 },
    ...
    return NextResponse.json({ message: "OTP sent to your email", tempToken }, { status: 200 });
  } catch (error) {
