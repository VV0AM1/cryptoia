...
export function PriceVolumeChart({ candles, series, positive, interval, onIntervalChange, loading }:{
  candles: Candle[];
  ...
  return (
    <div className="bg-zinc-800 rounded-2xl p-4">
      <div className="flex items-center justify-between mb-2">
        ...
        <div className="flex items-center gap-1 text-xs">
          {["1m","5m","15m","1h","4h","1d"].map(iv => (
            <button key={iv} onClick={() => onIntervalChange(iv)} className={`px-2 py-1 rounded-lg border ${interval===iv?"bg-zinc-700 border-zinc-600":"border-zinc-700 hover:bg-zinc-700/50"}`}>{iv}</button>
          ))}
          {loading && <span className="ml-2 text-zinc-400">loading…</span>}
        </div>
      </div>
      <div className="h-72 lg:h-[420px] w-full">
      ...
      <div className="mt-3 text-xs text-zinc-400">RSI(14): <span className="text-white">{Number.isFinite(lastRSI) ? lastRSI.toFixed(1) : "—"}</span></div>
    </div>
  );
}