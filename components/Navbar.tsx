...
export default function NavBar() {
  ...
  const handleCoinSelect = async (coin: any) => {
    try {
      const res = await fetch(`/api/market/coin/${coin.id}`);
  ...
  const handleTransactionComplete = () => {
    ...
    refetch(); // ✅ Refresh portfolio summary
  };
      ...
      <div className="hidden md:flex items-center space-x-3 ml-4">
        {session?.user ? (
          <>
            ...
            {/* Favorites & Avatar */}
            <motion.button whileTap={{ scale: 0.9 }} whileHover={{ scale: 1.15 }} className="text-yellow-400 hover:text-yellow-300" title="Favorites">
          ...
          <>
            ...
            <Link href="/login" className="text-sm px-4 py-2 border border-zinc-600 rounded-md hover:bg-zinc-700 transition">Login</Link>
            <Link href="/register" className="text-sm px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition">Register</Link>
          </>
        )}
      </div>
        ...
        <AnimatePresence>
          {menuOpen && (
            ...
            >
              {session?.user ? (
                <>
                  <Link href="/dashboard" className="block text-sm px-4 py-2 rounded-md hover:bg-zinc-700 transition">Dashboard</Link>
                  <button
                    onClick={() => {
                      signOut();
                      setMenuOpen(false);
                ...
                <>
                  <Link href="/login" className="block text-sm px-4 py-2 border border-zinc-600 rounded-md hover:bg-zinc-700 transition">Login</Link>
                  <Link href="/register" className="block text-sm px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition">Register</Link>
                </>
              )}
            </motion.div>
          )}
